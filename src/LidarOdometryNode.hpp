...
public:
  explicit LidarOdometryNode(
    ...
    config.voxel_resolution_alpha =
        get_parameter("voxel_resolution_alpha").as_double();
    config.voxel_resolution_beta =
    ...
    config.initial_threshold =
        get_parameter("/threshold/initial_threshold").as_double();
    config.min_motion_threshold =
        get_parameter("/threshold/min_motion_threshold").as_double();
    config.fixed_threshold =
        get_parameter("/threshold/fixed_threshold").as_double();
    ...
    orientation_covariance_ =
        get_parameter("orientation_covariance").as_double();
    ...
    point_cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "points", 10,
        std::bind(&LidarOdometryNode::pointCloudCallback, this,
                  std::placeholders::_1));

    odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("lid_odom", 10);
    map_pub_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>("voxel_map", 10);
    cloud_pub_ =
...
private:
  void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    if (!lidar_pose_acquired) {
      ...
      try {
        geometry_msgs::msg::TransformStamped transform_stamped =
            buffer_.lookupTransform(lidar_link_id_, base_frame_id_,
                                    tf2::TimePointZero);
    ...
    if (!timestamps.empty() && timestamps.size() == points.size()) {
      unskewed_points =
          cloud::motionDeSkew(transformed_points, timestamps, pose_diff_);
    } else {
  ...
  transformPointCloud(const std::vector<Eigen::Vector3d> &points,
  ...
  Sophus::SE3d
      current_lidar_pose; // latest fused pose between imu and lidar_data
